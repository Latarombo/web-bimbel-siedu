...
const globalForDb = globalThis as unknown as {
  __sieduDb?: Db;
  __sieduDbHash?: string;
};

const contractHash = contractJson.storage?.storageHash ?? '';

export const db: Db = (
  globalForDb.__sieduDb && globalForDb.__sieduDbHash === contractHash
    ? globalForDb.__sieduDb
    : (() => {
        globalForDb.__sieduDbHash = contractHash;
        return (globalForDb.__sieduDb = postgres<Contract>({
          contractJson,
