export default async function Page({ params }: { params: Promise<Record<string,string>> }) {
  ...
  return (
    <div style={{padding:24}}>
      <h1 style={{fontSize:24,fontWeight:700}}>Class Detail</h1>
      <p style={{color:'#666',marginTop:8}}>Route: <code>/classes/[id]</code></p>
      <pre style={{marginTop:12,background:'#f5f5f5',padding:12}}>{JSON.stringify(p,null,2)}</pre>
      <p style={{marginTop:12}}>Placeholder — Class Detail. Desain menyusul.</p>
