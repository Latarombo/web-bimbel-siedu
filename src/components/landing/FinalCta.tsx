...
export default async function FinalCta() {
 ...
 <div className="relative grid items-center gap-6 rounded-3xl border border-blue-100 bg-white/80 p-6 sm:p-10 lg:grid-cols-[1.1fr_0.9fr] lg:items-stretch lg:gap-4">
 {/* Kolom kiri — copy */}
 <div className="order-2 text-center lg:order-1 lg:text-left">
 <h2 className="text-balance text-3xl sm:text-4xl font-bold leading-tight tracking-tight text-slate-900">
 {tr("text71")}{" "}
 <span className="text-brand">{tr("text73")}</span>
 </h2>
 <p className="mx-auto mt-4 max-w-xl text-sm sm:text-base leading-relaxed text-slate-500 lg:mx-0">
 {tr("text74")}</p>
 <div className="mt-8 flex flex-col justify-center gap-3 sm:flex-row sm:gap-4 lg:justify-start">
 <Link
 href="/contact"
 className="inline-flex items-center justify-center rounded-lg border-2 border-gray-200 bg-white px-8 py-3.5 text-sm font-bold text-slate-800 transition-colors hover:border-gray-300 hover:bg-gray-50"
 >
 {tr("text75")}</Link>
 <Link
 href="/register"
 ...
 >
 {tr("text76")}<ArrowRight className="size-4" />
 </Link>
 </div>
 </div>
 ...
 <div className="hidden lg:order-2 lg:block lg:min-h-[260px]" aria-hidden="true" />
